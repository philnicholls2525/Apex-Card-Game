...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case "social.profile": data = await rpc("api_social_profile", { p_user: userId, p_target: String(body.targetUserId || "") }); break;
      ...
      case "friend.cancel": data = await rpc("api_friend_cancel", { p_user: userId, p_friendship: String(body.friendshipId || "") }); break;
      ...
      case "friendTeam.status": data = await rpc("api_friend_team_status", { p_user: userId }); break;
      case "friendTeam.create": data = await rpc("api_friend_team_create", { p_user: userId, p_name: String(body.name || ""), p_tag: String(body.tag || ""), p_description: String(body.description || ""), p_key: idempotencyKey }); break;
      case "friendTeam.invite": data = await rpc("api_friend_team_invite", { p_user: userId, p_team: String(body.teamId || ""), p_target: String(body.targetUserId || ""), p_key: idempotencyKey }); break;
      case "friendTeam.respond": data = await rpc("api_friend_team_respond", { p_user: userId, p_invite: String(body.inviteId || ""), p_accept: Boolean(body.accept) }); break;
      case "friendTeam.cancelInvite": data = await rpc("api_friend_team_cancel_invite", { p_user: userId, p_invite: String(body.inviteId || "") }); break;
      case "friendTeam.leave": data = await rpc("api_friend_team_leave", { p_user: userId, p_team: String(body.teamId || "") }); break;
      case "friendTeam.removeMember": data = await rpc("api_friend_team_remove_member", { p_user: userId, p_team: String(body.teamId || ""), p_target: String(body.targetUserId || "") }); break;
      case "competition.overview": data = await rpc("api_competition_overview", { p_user: userId }); break;
